...
public class writeBack {
	...
	public writeBack() {
		choiceMade = false;//to store info if the choice for writing back or copying the original file was made 
	}
	
	public void cleanReview(TrialObject a, ParseReview parser, int i, String path) throws ParserConfigurationException, IOException, SAXException {
		    ...
		    try {
				...
				Document rm5 = parser.rm5[i];
				    ...
				    p.appendChild(br);
				    p.appendChild(rm5.createTextNode("---------------- Appended outcomes -----------------"));
				...
				//String path = "C:\\NewReviews\\" + a.charObject.reviewTitle + ".rm5"; use this instead of the path argument given above if you want to create a copy of the review. if only one character of the path is altered, a copy will be made automatically, and, the original file is not harmed. 
				
				
				
				if(!choiceMade) {//once an export method was chosen, the choice stays on for the rest of the process. Therefore, the dialouge will not be called again if choiceMade is true.
					choice = exportMessage();//choose export option
					choiceMade = true;//choice was made and will not be asked again
					
				}
				
				if(!choice) {//if choice is false, path is changed
					path = path.replace(".rm5", "_NEW.rm5");
				}
				
				StreamResult result = new StreamResult(new File(path));//using path write the new file
	...
	protected boolean exportMessage(){
        
        String[] options = {"Original","Copy"};
        int choice = JOptionPane.showOptionDialog(null, //Component parentComponent
               "If you want to write the outcomes to the original file, press 'Original'.\nIf you want a copy (which will be called '[your original review]_NEW'.rm5), press 'Copy'.", //Object message,
               "Choose export option", //String title
